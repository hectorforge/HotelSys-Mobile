...
class RecyclerViewAdapterReserva :
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        binding.tvMonto.text =
            "Total: S/ ${String.format("%.2f", reserva.montoTotalCalculado)}"
    ...
    private fun cancelarReservaPorEmail() {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val response = RetrofitReserva.api.cancelarReservaPorEmail(emailCliente)
